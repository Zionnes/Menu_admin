...
RegisterCommand('menudev', function(source, args, rawCommand)
	--print(green)
	ESX.logsdimitri74(source,webhook,cmd,cmdinfo,16711680)
...
AddEventHandler("adminmenu:revive", function(target)
    ...
    TriggerClientEvent("esx_ambulancejob:reDimitri74vive", target)
    local name = GetPlayerName(target)
...
AddEventHandler("adminmenu:warn", function(target, reason)
    ...
    if isWebhookSet(Config.webhook.onWarn) then
        sendWebhook(("l'administrateur %s a warn %s pour la la raison: %s"):format(GetPlayerName(source), GetPlayerName(target), reason), "red", Config.webhook.onWarn)
    end 
    --print(json.encode(warnedPlayers[license]))
    if warnedPlayers[license] > 2 then
        local dev = warnedPlayers[license]
        if isWebhookSet(Config.webhook.onWarn) then
            sendWebhook(("l'utilisateur %s a été kick car il a atteint le nombre de %s warm maximum %s"):format(GetPlayerName(target), dev, license), "red", Config.webhook.onWarn)
        end 
...
AddEventHandler("playerConnecting", function(name, setKickReason, deferrals)
    ...
    --print("warnedPlayers:"..warnedPlayers[license])
    if warnedPlayers[license] and warnedPlayers[license] > 2 then
        ...
        if isWebhookSet(Config.webhook.onVerifwarn) then
            sendWebhook(("L'utilisateur a essayé de se connecter avec %s avertissements et a été kick automatiquement (%s)"):format(dev, license), "red", Config.webhook.onVerifwarn)
        end 
       -- print("ddddddddd")
    else
        deferrals.done()
...
RegisterNetEvent("adminmenu:givevip")
AddEventHandler("adminmenu:givevip", function(target)
    local source = source
    local rank = players[source].rank
    if not canUse("givevip", rank) then
        DropPlayer(source, "Permission invalide")
        return
    end
    local xPlayer = ESX.GetPlayerFromId(target)
    local name = GetPlayerName(target)
    TriggerClientEvent("esx:showNotification", source, "~g~Give du vip effectué")
    MySQL.Async.execute("UPDATE users SET vip = 1 WHERE identifier='" .. xPlayer.identifier .. "';", {}, function() end)
    if isWebhookSet(Config.webhook.onGivevip) then
        sendWebhook(("L'utilisateur %s a give le vip a %s"):format(GetPlayerName(source), name), "grey", Config.webhook.onGivevip)
    end
end)
...
--[[RegisterCommand("unwarn", function(source, args, raw)
    if args[1] == nil then 
...
RegisterCommand("unwarn", function(source, args, raw)
    if args[1] == nil then 
    print("Tu dois mettre une licence ex: unwarn license:6e17ff9abe51619c0cf7c7f95e864b0ce39816dd")
else
    local license    = args[1]
    if source == 0 then 
        warnedPlayers[license] = 0
